import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { getAllItems } from '../services/apiService';
import '../styles/ViewItemsScreen.css';

function ViewItemsScreen() {
  const navigate = useNavigate();
  const [items, setItems] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [errorMessage, setErrorMessage] = useState('');

  useEffect(() => {
    fetchItems();
  }, []);

  const fetchItems = async () => {
    setIsLoading(true);
    setErrorMessage('');
    
    try {
      const data = await getAllItems();
      setItems(data);
    } catch (error) {
      setErrorMessage(error.message);
    } finally {
      setIsLoading(false);
    }
  };

  const handleReportItem = () => {
    navigate('/');
  };

  const formatDate = (dateString) => {
    const date = new Date(dateString);
    return date.toLocaleString('en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
      minute: '2-digit',
    });
  };

  return (
    <div className="screen-container">
      <div className="app-bar">
        <h1>All Found Items</h1>
        <button
          className="add-button"
          onClick={handleReportItem}
          title="Report new item"
        >
          <svg fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z"
              clipRule="evenodd"
            />
          </svg>
        </button>
      </div>

      <div className="content">
        {isLoading ? (
          <div className="loading-container">
            <div className="loading-spinner" />
            <p>Loading items...</p>
          </div>
        ) : errorMessage ? (
          <div className="error-container">
            <svg className="error-icon" fill="currentColor" viewBox="0 0 20 20">
              <path
                fillRule="evenodd"
                d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
                clipRule="evenodd"
              />
            </svg>
            <h2>Error Loading Items</h2>
            <p>{errorMessage}</p>
            <button className="primary-button" onClick={fetchItems}>
              Try Again
            </button>
          </div>
        ) : items.length === 0 ? (
          <div className="empty-container">
            <svg className="empty-icon" fill="currentColor" viewBox="0 0 20 20">
              <path
                fillRule="evenodd"
                d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z"
                clipRule="evenodd"
              />
            </svg>
            <h2>No Items Found</h2>
            <p>No items have been reported yet.</p>
            <button className="primary-button" onClick={handleReportItem}>
              Report First Item
            </button>
          </div>
        ) : (
          <>
            <div className="items-header">
              <h2>Found Items ({items.length})</h2>
              <button className="refresh-button" onClick={fetchItems}>
                <svg fill="currentColor" viewBox="0 0 20 20">
                  <path
                    fillRule="evenodd"
                    d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z"
                    clipRule="evenodd"
                  />
                </svg>
                Refresh
              </button>
            </div>

            <div className="items-grid">
              {items.map((item) => (
                <div 
                  key={item.id} 
                  className="item-card"
                  onClick={() => navigate(`/item/${item.id}`)}
                  style={{ cursor: 'pointer' }}
                >
                  <div className="item-header">
                    <div className="item-id">#{item.id}</div>
                    <div className={`item-category category-${item.category.toLowerCase().replace(/\s+/g, '-')}`}>
                      {item.category}
                    </div>
                  </div>

                  <div className="item-description">
                    {item.description}
                  </div>

                  <div className="item-footer">
                    <div className="item-meta">
                      <svg className="meta-icon" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fillRule="evenodd"
                          d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"
                          clipRule="evenodd"
                        />
                      </svg>
                      <span>{item.founder_id}</span>
                    </div>

                    <div className="item-meta">
                      <svg className="meta-icon" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fillRule="evenodd"
                          d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z"
                          clipRule="evenodd"
                        />
                      </svg>
                      <span>{item.question_count} question{item.question_count !== 1 ? 's' : ''}</span>
                    </div>
                  </div>

                  <div className="item-date">
                    <svg className="date-icon" fill="currentColor" viewBox="0 0 20 20">
                      <path
                        fillRule="evenodd"
                        d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"
                        clipRule="evenodd"
                      />
                    </svg>
                    {formatDate(item.created_at)}
                  </div>
                </div>
              ))}
            </div>
          </>
        )}
      </div>
    </div>
  );
}

export default ViewItemsScreen;
